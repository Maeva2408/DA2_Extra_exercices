...
# Import raw data
data_in <- "C:/Users/mbrae/OneDrive/Bureau/CEU/DA2/DA2_Extra_exercices/data"
...
hotel_brussels <- filter( hotel_brussels , 
                        year == 2017 & month == 11 & weekend == 0,
                        accommodation_type == "Hotel" , 
                        stars >= 3 & stars <= 4)

# Writing out csv
write_csv( hotel_brussels , paste0( data_in,
                                  "/clean/hotel_Brussels.csv"))

# Create descriptive table
brussels_sum_stat <- summarise( hotel_brussels , 
                              mean = mean( price ),
                              median = median( price ),
                              std = sd( price ),
                              min = min( price ),
